...
class SheerIDVerifier:
    ...
    def create_verification(self) -> str:
        """Create a fresh verificationId for Spotify"""
        body = {
        ...
        if status != 200 or not isinstance(data, dict) or not data.get("verificationId"):
            raise Exception(f"Failed to create new Spotify verification (Status {status}): {data}")
    ...
    def verify(
        ...
        try:
            ...
            if should_do_step_2:
                ...
                time.sleep(random.uniform(3.8, 6.9))
                step2_body = {
