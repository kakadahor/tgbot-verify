"""Concurrency Control Tools (Optimized)

Performance improvements:
1. Dynamic concurrency limiting (based on system load)
2. Segregated concurrency control for different verification types
...
def _calculate_max_concurrency() -> int:
    ...
    try:
        ...
        # Each GB of RAM supports 2 concurrent tasks
        ...
        logger.warning(f"Unable to fetch system resource info: {e}, using default value")
...
def get_verification_semaphore(verification_type: str) -> asyncio.Semaphore:
    """Get the semaphore for the specified verification type
    
    Args:
        verification_type: The type of verification
        
    Returns:
        asyncio.Semaphore: The corresponding semaphore
    """
...
def adjust_concurrency_limits(multiplier: float = 1.0):
    ...
    logger.info(
        f"Adjusted concurrency limits: multiplier={multiplier}, "
