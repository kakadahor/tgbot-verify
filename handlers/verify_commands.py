...
async def verify4_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    """Handle /verify4 command - Bolt.new Teacher (auto-retrieve code version)"""
    user_id = update.effective_user.id
    ...
    try:
        ...
        await processing_msg.edit_text(
            ...
            f"(Max wait: 20 seconds)"
        )
        
        # Step 2: Auto-retrieve verification code (max 20 seconds)
...
async def _auto_get_reward_code(
    ...
    """Auto-retrieve verification code (lightweight polling, does not affect concurrency)
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        while True:
            ...
            # Check if timeout
            if elapsed >= max_wait:
                logger.info(f"Auto-retrieve code timeout ({elapsed}s), let user query manually")
                return None
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if current_step == "success":
                        ...
                        if code:
                            logger.info(f"✅ Auto-retrieve code success: {code} (elapsed {elapsed}s)")
                            return code
                ...
                logger.warning(f"Query verification code error: {e}")
                await asyncio.sleep(interval)
...
async def getV4Code_command(update: Update, context: ContextTypes.DEFAULT_TYPE, db: Database):
    ...
    try:
        ...
        logger.error("Get Bolt.new verification code failed: %s", e)
        await processing_msg.edit_text(
